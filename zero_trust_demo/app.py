...
def verify_password(username: str, password: str) -> bool:
    ...
    # Use constant time comparison to prevent timing attacks (though hash comparison is fast)
...
def abac_authorize(claims: dict, action: str, resource: str) -> bool:
    ...
    Decides validation based on User Role, Env Attributes (Time, IP), and Resource Attributes.
    """
    ...
    logger.debug(f"ABAC Check: User={claims.get('sub')}, Role={role}, Action={action}, Resource={resource}, Time={current_hour}, IP={ip}")

    # Policy 1: Admin can access anytime, anywhere (simulating broad access, maybe restrict later)
    if role == "admin":
        # Example constraint: Admin only between 6 AM and 10 PM UTC
        allowed = 6 <= current_hour <= 22
        if not allowed:
            logger.info(f"Access Denied: Admin {claims.get('sub')} outside working hours.")
        return allowed

    # Policy 2: Regular User constraints
    if role == "user":
        # Strict context-aware check
        # Must be: 'read' action, 'data' resource, corporate device (laptop*), corporate IP (10.0.0.*)
        ...
        if is_read_data and is_corp_device and is_corp_ip:
            ...
            logger.info(f"Access Denied: User {claims.get('sub')} failed policy check. ReadData={is_read_data}, Device={is_corp_device}, IP={is_corp_ip}")
            return False
...
def login():
    ...
    if not verify_password(body.username, body.password):
        logger.warning(f"Failed login attempt for user: {body.username} (Invalid Credentials)")
...
def refresh():
    ...
    new_access_token = issue_access_token(username, role, {"ip": ip, "device": body.device})
